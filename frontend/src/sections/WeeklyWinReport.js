  ...
  return (
    <div className="card p-6 md:p-8 mt-8" data-testid="weekly-win-report">
      ...
      <div className="flex items-center gap-1.5 mt-2 text-sm" data-testid="win-pos-import"
        style={{ color: r.posImport.importedThisWeek ? "var(--success)" : "var(--danger)" }}>
        {r.posImport.importedThisWeek ? <CheckCircle2 size={15} /> : <AlertTriangle size={15} />}
        <span className="font-semibold">
          {r.posImport.importedThisWeek
            ? `POS CSV imported during the week (${r.posImport.importsInWeek}×) — numbers reconciled.`
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mt-4">
        <div data-testid="win-stat-redeemed">
          ...
          <div className="mono text-4xl mt-1">{r.current.redeemed}</div>
          <Delta value={r.deltas.redeemed} />
        </div>
        <div data-testid="win-stat-revenue">
          <Overline>Revenue Proven</Overline>
          <div className="mono text-4xl mt-1" style={{ color: "var(--success)" }}>{money(r.current.revenue)}</div>
          <Delta value={r.deltas.revenue} isMoney />
        </div>
        <div data-testid="win-stat-adspend">
          <Overline>Ad Spend</Overline>
          <div className="mono text-4xl mt-1" style={{ color: "var(--text-secondary)" }}>{money(r.adSpend.total)}</div>
          <span className="text-xs mono mt-1 inline-block" style={{ color: "var(--text-secondary)" }}>
            vs {money(r.adSpend.prevTotal)} prior wk
          </span>
        </div>
      </div>
      <div className="flex flex-wrap gap-x-8 gap-y-2 mt-4">
        {secondary.map((s) => (
          <div key={s.key} data-testid={`win-stat-${s.key}`} className="flex items-baseline gap-2">
            ...
            <span className="mono text-lg">{r.current[s.key]}</span>
            <Delta value={r.deltas[s.key]} />
          </div>
        ))}
      </div>

      {/* Prize payouts — reconcile vs POS */}
      {r.prizeBreakdown.length > 0 && (
      ...
      <p className="text-xs mt-3" style={{ color: "var(--text-secondary)" }} data-testid="win-so-far">
        This week so far: {r.soFar.spins} spins · {r.soFar.newMembers} new members · {r.soFar.redeemed} redeemed
        · {money(r.soFar.revenue)} proven. Campaigns can start any day — the week closes hard on Sunday.
      </p>
